...
st.markdown(
    ...
    h1.main-title { font-size:40px; font-weight:800; margin:0; color:#FFF7F2; } /* BIG & BOLD TITLE */
...
# -------------------------
# Main layout: map and visualizations (LEFT main map + RIGHT chart selector)
# -------------------------
...
with left:
    ...
    # Bubble chart (left map area - optional)
    if show_bubble:
        st.markdown("<div class='card'>", unsafe_allow_html=True)
...
# -------------------------
# RIGHT column: Chart selector + display (Option C)
# -------------------------
with right:
    st.markdown("<div class='card'>", unsafe_allow_html=True)
    st.subheader("📌 Right-panel Chart Selector")

    chart_options = [
        "Top Countries by Fatalities (Bar)",
        "Attack Types (Pie)",
        "Fatalities vs Injuries (Bubble)",
        "Incidents Over Time (Line)",
        "Heatmap — Year vs Top Countries",
        "Attack Types (Donut)",
        "Fatalities vs Injuries (Scatter)",
        "Correlation (Numeric Features)"
    ]
    chosen = st.selectbox("Select chart to display", chart_options, key="right_chart_select")

    st.markdown("---")

    # 1) Top Countries by Fatalities (bar)
    if chosen == "Top Countries by Fatalities (Bar)":
        ...
        if "Country" in df.columns and "Fatalities" in df.columns:
            ...
            st.info("Columns 'Country' and 'Fatalities' required for this chart.")

    # 2) Pie: Attack Types
    elif chosen == "Attack Types (Pie)":
        st.subheader("🧭 Attack Types (Pie)")
        if "AttackType" in df.columns:
            pie = df["AttackType"].value_counts().reset_index()
            pie.columns = ["AttackType", "Count"]
            if pie.empty:
                st.info("No attack type data.")
            else:
                fig_p = px.pie(pie, names="AttackType", values="Count", title="Attack Type Distribution")
                st.plotly_chart(fig_p, use_container_width=True)
        else:
            st.info("Column 'AttackType' not found in dataset.")

    # 3) Bubble (same as left but here if chosen)
    elif chosen == "Fatalities vs Injuries (Bubble)":
        st.subheader("🔵 Fatalities vs Injuries (Bubble)")
        if {"Fatalities", "Injuries", "Incidents", "Country"}.issubset(df.columns):
            bubble_df = df.groupby("Country", as_index=False).agg({"Fatalities": "sum", "Injuries": "sum", "Incidents": "sum"})
            bubble_df = bubble_df[(bubble_df["Fatalities"] + bubble_df["Injuries"]) > 0]
            if bubble_df.empty:
                st.info("No bubble data for selected filters.")
            ...
            st.info("Required columns missing (Fatalities, Injuries, Incidents, Country).")

    # 4) Line: Incidents over time
    elif chosen == "Incidents Over Time (Line)":
        st.subheader("📈 Incidents Over Time")
        if "Year" in df.columns and "Incidents" in df.columns:
            inc_trend = df.groupby("Year")["Incidents"].sum().reset_index()
            if inc_trend.empty:
                st.info("No incidents data.")
            else:
                fig_l = px.line(inc_trend, x="Year", y="Incidents", markers=True, title="Incidents Over Years")
                fig_l.update_layout(margin=dict(t=10, b=0))
                st.plotly_chart(fig_l, use_container_width=True)
        else:
            st.info("Columns 'Year' and 'Incidents' required for this chart.")

    # 5) Heatmap
    elif chosen == "Heatmap — Year vs Top Countries":
    ...
    # 6) Donut chart (attack types)
    elif chosen == "Attack Types (Donut)":
        st.subheader("🍩 Attack Types (Donut)")
        if "AttackType" in df.columns:
            donut = df["AttackType"].value_counts().reset_index()
            donut.columns = ["AttackType", "Count"]
            if donut.empty:
                st.info("No attack type data.")
            else:
                fig_d = px.pie(donut, names="AttackType", values="Count", hole=0.4, title="Attack Type Share (Donut)")
                st.plotly_chart(fig_d, use_container_width=True)
        else:
            st.info("Column 'AttackType' not found in dataset.")

    # 7) Scatter: Fatalities vs Injuries
    elif chosen == "Fatalities vs Injuries (Scatter)":
        st.subheader("⚖️ Fatalities vs Injuries (Scatter)")
        if {"Fatalities", "Injuries"}.issubset(df.columns):
            scatter_df = df.groupby("Country", as_index=False).agg({"Fatalities": "sum", "Injuries": "sum"})
            if scatter_df.empty:
                st.info("No data for scatter.")
            else:
                fig_s = px.scatter(scatter_df, x="Injuries", y="Fatalities", hover_name="Country", size_max=12, title="Fatalities vs Injuries")
                fig_s.update_layout(margin=dict(t=10, b=0))
                st.plotly_chart(fig_s, use_container_width=True)
        else:
            st.info("Columns 'Fatalities' and 'Injuries' required for this chart.")

    # 8) Correlation
    elif chosen == "Correlation (Numeric Features)":
